...
def main():
    ...
    file_path= r"C:\Users\Victor\Documents\GitHub\Projeto-Mestrado\topologia.json"
    ...
    #Extrai particionamento de cada FPGA:

    lista_particionamento = []
    lista_recursos = []

    for idx,modelo in enumerate(lista_fpgas):
        aux = []
        for part in modelo:
            if part == 'Modelo':
                continue
            clb= modelo[part]["CLBs"]
            bram= modelo[part]["BRAM"]
            dsp= modelo[part]["DSP"]
            aux.append((clb,bram,dsp))
        lista_recursos.append(aux)

    for node_idx, node in enumerate(graph.values()):
        if node["Resources"] != [(0, 0, 0)]:
            for idx,data in enumerate(lista_recursos):
                if node["Resources"] == data:
                    lista_particionamento.append(idx)
    ...
    # Define posicionamento das FPGAs
    lista_nodos = []
    for node_idx, node in enumerate(graph.values()):
        if node["Resources"][0] != (0, 0, 0):
            lista_nodos.append(f"Nodo_{node_idx}")
    ...
    for req_idx, r in enumerate(requisitions):
        ...
        for k in possible_paths:
            ...
            lat_run = gp.quicksum(r[6][f_idx] * x.get((req_idx, tuple(f), f'Nodo_{n}', p_idx, tuple(k)),0) 
                                    ...
                                    for n in k
                                    for p_idx in range(len(particoes)))
            model.addConstr((lat_path + lat_run) <= r[2] * z[req_idx, tuple(k)], 
                            name=f"latencia_requisicao_{req_idx}_caminho_{k}")
            
    # Restrição 7: Limitação de recursos computacionais
    for n in nodes:
        for p_idx, part in enumerate(particoes):
            clb_total = gp.quicksum(
                x.get((req_idx, tuple(f), n, p_idx, tuple(k)), 0) * f[0]
                for req_idx, r in enumerate(requisitions)
                for f in r[4]
                for k in dfs_caminhos(paths, r[0], r[1])
            )
            bram_total = gp.quicksum(
                x.get((req_idx, tuple(f), n, p_idx, tuple(k)), 0) * f[1]
                for req_idx, r in enumerate(requisitions)
                for f in r[4]
                for k in dfs_caminhos(paths, r[0], r[1])
            )
            dsp_total = gp.quicksum(
                x.get((req_idx, tuple(f), n, p_idx, tuple(k)), 0) * f[2]
                for req_idx, r in enumerate(requisitions)
                for f in r[4]
                for k in dfs_caminhos(paths, r[0], r[1])
            )

            model.addConstr(clb_total <= part[0], name=f"limite_clb_{n}_{p_idx}")
            model.addConstr(bram_total <= part[1], name=f"limite_bram_{n}_{p_idx}")
            model.addConstr(dsp_total <= part[2], name=f"limite_dsp_{n}_{p_idx}")
    ...
    #Restrição 16: Determinar particionamento

    count = set(lista_particionamento)
    filtered_last = [last[i] for i in count if i < len(last)]

    for idx, val in enumerate(count):
        p_idx = filtered_last[idx]
        model.addConstr(gp.quicksum(w.get((n, p_idx),0) for n in nodes ) >= lista_particionamento.count(val), 
                        name=f"particionamento_{count}")         
